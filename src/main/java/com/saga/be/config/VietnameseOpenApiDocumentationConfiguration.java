...
public class VietnameseOpenApiDocumentationConfiguration {
    ...
    private static final Map<String, Documentation> SPECIAL_OPERATIONS = Map.ofEntries(
            ...
            entry("AuthController#me", "Xem thông tin tài khoản đang đăng nhập", "Trả hồ sơ, vai trò, trạng thái và avatarUrl đồng bộ từ OIDC picture nếu claim có mặt. Không truyền userId và không dùng Bearer token."),
            entry("CourseContributionWeightController#updateCurrentWeights", "Cập nhật trực tiếp trọng số đóng góp của khóa học", "Chỉ LECTURER phụ trách exact Course. Actor lấy từ session localProfileId; request không nhận lecturerId. Thang điểm Course hiện hành tổng 100. Không thay ProjectGroupWeight hay công thức Contribution."),
            entry("LecturerAnalyticsController#progress", "Xem tiến độ của sinh viên", "ADMIN đọc mọi Course; LECTURER chỉ Course mình phụ trách; STUDENT MEMBER chỉ đọc chính mình; STUDENT LEADER đọc sinh viên thuộc exact Team của mình; MENTOR và cross-Team/cross-Course bị từ chối."),
            ...
            entry("AuthController#sessionEvents", "Nhận sự kiện phiên làm việc", "Luồng SSE theo JSESSIONID. Student và Lecturer nhận đúng một event account-disabled với code ACCOUNT_DISABLED khi tài khoản không còn ACTIVE; Admin chỉ nhận heartbeat. Không nhận Bearer, CSRF hay định danh từ client. FCM không phải kênh thu hồi phiên."),
    ...
    private static final Map<String, String> SUMMARY_OVERRIDES = Map.ofEntries(
            ...
            summary("AuthController#sessionEvents", "Nhận sự kiện phiên làm việc"),
