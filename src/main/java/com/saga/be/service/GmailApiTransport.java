...
public class GmailApiTransport {
    ...
    public static final String SEND_URI =
    ...
    private GmailDeliveryException translateTokenFailure(RestClientResponseException exception) {
    ...
    private GmailDeliveryException translateGmailFailure(RestClientResponseException exception) {
        int status = exception.getStatusCode().value();
        if (status == 401) {
    ...
    private GmailDeliveryException failure(
            String category,
            boolean retryable,
            Integer httpStatus,
            Throwable exception
    ) {
        return new GmailDeliveryException(category, retryable, httpStatus, exception);
    }
    ...
    private String encodeRawMessage(GmailMessage message) {
