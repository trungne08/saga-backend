...
The claim transaction commits before the delivery adapter is called, so it does not hold a database lock during provider I/O. Delivery is **at-least-once**, not exactly-once: if a process stops after Gmail API accepts a message but before `SENT` is persisted, stale recovery can make a later attempt. `users.messages.send` does not receive an application idempotency key, so duplicate delivery remains possible in this crash window.
...
| `app.student-invitation.gmail-api.client-id` | `GMAIL_API_CLIENT_ID` | OAuth client id; required |
| `app.student-invitation.gmail-api.client-secret` | `GMAIL_API_CLIENT_SECRET` | OAuth client secret; required deployment secret |
| `app.student-invitation.gmail-api.refresh-token` | `GMAIL_API_REFRESH_TOKEN` | offline refresh token granted with `gmail.send`; required deployment secret |
| `app.student-invitation.gmail-api.sender-email` | `GMAIL_API_SENDER_EMAIL` | Gmail account or authorized send-as address used in FROM; required |
| `app.student-invitation.gmail-api.sender-name` | `GMAIL_API_SENDER_NAME` | UTF-8 display name used in FROM; required |

The CTA URL is configuration-driven; `/auth/callback` is not used to begin login. `GmailApiStudentInvitationDeliveryAdapter` refreshes an access token over HTTPS and posts Base64URL RFC/MIME content to Gmail `users.messages.send`. It reuses Spring `RestClient` + JDK HTTP with bounded integration timeouts (3/10 seconds by default), caches the access token in process with a 60-second expiry margin and never calls Google during startup or health checks. When any of the five Gmail settings is missing, configuration selects the unavailable adapter without failing application startup or inventing credentials.

The least-privilege OAuth scope is `https://www.googleapis.com/auth/gmail.send`. Runtime code must not persist or log client secret, refresh/access token, Authorization header, token form, MIME `raw`, recipient/body or provider response. HTTP 429/5xx/network and 403 rate/quota reasons are classified retryable; revoked/invalid OAuth, malformed responses, 400/401 and 403 permission/sender are non-retryable. The existing outbox schema does not persist this classification, so the processor still records every failure as `FAILED` and the existing scheduler may retry it up to the five-attempt cap. Source/test behavior is **CONFIRMED_SOURCE_TEST**; real Gmail/Railway delivery remains **TBD_DEPLOYMENT_SMOKE**.
...
Tests cover matching/conflicts/status/idempotency, competitive bind, multi-course role preservation, import rollback/dedup, linked/unlinked text+HTML, mocked OAuth refresh + Gmail send HTTPS, token cache, missing-config fallback, MIME Base64URL, provider status/reason mapping, concurrent claims, stale recovery, retry, SENT no-resend and membership preservation. `CourseTeamMembershipGuardIntegrationTest` also covers same-Team idempotency/role preservation, same-Course conflict 409, independent roles in different Courses, HTTP conflict and two independent competing transactions with a fresh final query. Targeted context/Gmail/invitation/import regression passes **10 suites / 70 tests / 0 failures / 0 errors**. Full suite runs **131 suites / 822 tests** with no Gmail failure; the pre-existing Course roster/DEC-023 drift remains. Full-order also exposed one unrelated Jira diagnostic error; the method passes isolated and no related source was changed.
