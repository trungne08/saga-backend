...
- **CONFIRMED_SOURCE_TEST (supersedes provider-TBD statement above):** Gmail SMTP adapter, Boot Mail dependency/configuration, text+HTML templates and fail-safe unavailable fallback exist. Actual production delivery is **TBD_DEPLOYMENT_SMOKE**.
...
## Update 2026-08-11 — Student Course Invitation qua Gmail SMTP

- **CONFIRMED_SOURCE_TEST:** `spring-boot-starter-mail` do Spring Boot quản lý và `GmailSmtpStudentInvitationDeliveryAdapter` dùng `JavaMailSender` để gửi MIME UTF-8 có plain-text fallback + HTML. `FROM` lấy từ `spring.mail.username`; recipient lấy duy nhất từ `StudentInvitationMessage.recipientEmail`.
- **CONFIRMED_SOURCE_TEST:** `StudentInvitationDeliveryConfiguration#studentInvitationDeliveryAdapter` chỉ chọn Gmail SMTP khi host, port, username, password, SMTP auth, STARTTLS và `JavaMailSender` đều sẵn sàng. Thiếu/bất toàn cấu hình chọn `UnavailableStudentInvitationDeliveryAdapter`; application vẫn start, không fake credential và không làm mail health probe tới localhost.
...
- **CONFIRMED_SOURCE_TEST:** outbox/claim/retry giữ nguyên. Chỉ adapter success mới chuyển `SENT`; provider/config failure chuyển `FAILED`, không rollback Student/Team/TeamMember. Safe log chỉ có `provider=GMAIL_SMTP`, stage, attempt, result, category và exception class; không log recipient/body/secret/identity. SMTP connect/read/write timeout hữu hạn lần lượt 5000/10000/10000 ms.
- **CONFIRMED_SOURCE_TEST (Gmail scope):** targeted invitation/import suite pass 37/37. Full `./mvnw.cmd clean test` chạy **116 suites / 753 tests / 1 failure / 0 errors / 0 skipped**; failure duy nhất là pre-existing roster/DEC-023 source drift, không phải Gmail delivery.
- **TBD_DEPLOYMENT_SMOKE:** chưa có bằng chứng Gmail/Railway production gửi và nhận thư thật. Cần deploy với secrets qua environment, quan sát PENDING/FAILED → SENT và inbox/spam; không ghi secret vào source/docs/log.
