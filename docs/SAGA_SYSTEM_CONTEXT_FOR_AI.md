...
- **CONFIRMED — Precedence, additive in front of the unchanged legacy classifier:** new `classifyTaskContribution(Task)` in both `TeamContributionService` and `ContributionCalculationService` checks the marker first, falls through unchanged to that service's own existing `classifyTaskSlice` (unchanged keyword lists, unchanged CODE-default for unlabeled Tasks, unchanged pick-first on keyword conflicts). The two services' legacy classifiers were already structurally slightly different before this change (different text-combination order/short-circuit) — reported as pre-existing technical debt, **not unified** (no test/source proves unifying them is safe).
...
- **Contracts:** OpenAPI **unchanged (151)** — no route added/removed. Migration head **V36 → V37**. Peer Review / Rubric / individual override / Student progress authorization / AI Agent / Jira-GitHub sync / ProjectType / COURSE-TEAM mode (DEC-088) **all unchanged**.
