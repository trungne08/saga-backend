## Current public Course and Contribution cleanup — 2026-08-17

- **CONFIRMED_SOURCE_TEST:** public Course list/detail/create/update use `CourseResponse`. Only `academicClass` is exposed; `clazz` and `academicClazz` are absent. JPA `Course.clazz`, `class_id`, and the Class table remain internal storage/model names.
- **CONFIRMED_SOURCE_TEST:** active public Contribution fields are CODE/TEST/DOCUMENT/RESEARCH only. No public schema exposes `designContributionWeight`, `designWeight`, DESIGN score/percentage, or proposed DESIGN weight. Legacy design columns and `V37` remain storage/history only; no migration or formula change is made for this cleanup.
- `ProjectType.DESIGN_ARCHITECTURE` remains an allowed, unrelated ProjectType catalog value. Browser auth/session/CSRF and COURSE/TEAM configuration mode are unchanged.
...
- **CONFIRMED_SOURCE_TEST:** `GET /api/v1/teams/{teamId}/contribution-graph` is a separate flowchart payload (criterion/student nodes, criterion→student edges with recognized tasks). Optional `sprintId` filters to that Sprint. Arithmetic is the SAGA mixer (DEC-092). Same read access as evaluation (DEC-095). No GHOSTING, no publish snapshot, no `/api/analytics/*`.
- **Contracts:** OpenAPI **152**. No migration.
...
- **CONFIRMED_SOURCE_TEST:** `SprintFirstContributionMixer` computes `slice = Σ SP_criterion × weight` (no intra-criterion share mix, unused weights stay unused). Sprint `% = (slice × P_s) / team adjust` (`P_s = 1` if that sprint has no peer). Project `% = (Σ slice × project P) / team adjust`. Evaluation response also returns pre-peer `sliceScore` / `sliceContributionPercentage`. Tasks with no sprint do not score.
- **Contracts:** OpenAPI **149**. No migration. Spec: `docs/CONTRIBUTION_CALCULATION_SPEC.md`.
...
- **CONFIRMED_SOURCE_TEST:** `TaskContributionClassifier` is labels-only. Exact reserved markers `saga:code`/`saga:test`/`saga:document`/`saga:research`. No keyword/title/`TaskType` fallback. Unlabeled or `AMBIGUOUS` Tasks enter no criterion; `adjustedSprintScore` still counts their story points.
- **CONFIRMED — DOCUMENT/RESEARCH attachment gate:** story points count only when the Task has at least one persisted Jira `TaskAttachment` **or** one `TaskWebLink`. Evidence count is not extra score. CODE/TEST attachments/links are ignored. SAGA `Document` rows are not a Contribution evidence source.
- **CONFIRMED — Jira attachment metadata ingestion:** `JiraProviderClientImpl` requests the `attachment` field; `JiraIssueUpsertService` replace-all syncs `task_attachment` (`V38__add_task_attachment.sql`). Missing/null/empty `attachment` = empty list. No file bytes, no content URL. GitHub attachments remain **not implemented**.
...
- **CONFIRMED_SOURCE_TEST — foundation piece of a larger milestone, NOT the full milestone:** the full ask was Jira attachment ingestion + GitHub Issue/comment attachment extraction + complete 4-criteria scoring. Only the provider-independent foundation was implemented; ingestion is **not built** — blocked on unresolved runtime TBDs (Jira attachment endpoint unconfirmed by live smoke test, GitHub App permission grant unconfirmed at runtime, private-repo attachment CDN auth path unproven). Do not claim external-evidence ingestion exists.
- **CONFIRMED — `ContributionCriterion` (`src/main/java/com/saga/be/service/contribution/ContributionCriterion.java`) = `CODE/TEST/DOCUMENT/RESEARCH`**, deliberately separate from `TaskType` (native MySQL `ENUM` column — extending it needs a physical migration, exactly as `V29` did for `REQUEST`) and from `DocumentType`. Never overload either with Contribution semantics.
- **CONFIRMED — Reserved markers (`ReservedContributionMarkerClassifier`):** `saga:code`, `saga:test`, `saga:document`, `saga:research` — exact case-sensitive string match only (`saga:test-extra`/`SAGA:TEST` do not match). >1 conflicting marker on a Task's labels = `AMBIGUOUS` (`ContributionMarkerClassification.conflicting()`), excluded from all four criteria until fixed — never pick-first. Unrelated business labels never trigger a conflict.
- **CONFIRMED — Precedence (historical DEC-089, superseded by DEC-090 labels-only):** previously marker-first then legacy `classifyTaskSlice`. DEC-090 removed the keyword fallback.
- **CONFIRMED — `TaskWeightConfig` audited and confirmed NOT a scoring authority:** its only consumer is `CourseService.softDeleteCourse`'s delete-dependency guard; never read by either Contribution service.
- **CONFIRMED — Task is the sole numeric Contribution authority when evidence is Task-linked — a real semantic change, not a no-op:** `NUMERIC_TASK_FORMULA_CHANGED = NO` — the per-Task formula is unchanged (`storyPoint ?? 1.0`, gated on `DONE` + correct assignee). `COMMIT_NUMERIC_CONTRIBUTION_CHANGED = YES` — the per-commit scoring loop that previously ALSO added `commit.getTask()`'s storyPoint into CODE/DOCUMENT (via the `commit.task` FK — audit confirmed no production upsert path ever writes it) was **removed entirely** from both services; a Task-linked commit now contributes exactly zero additional score, where it previously contributed its full weight. `OVERALL_CONTRIBUTION_SOURCE_SEMANTIC_CHANGED = YES` — deliberate product decision to close a real latent double-count path and pre-empt the same risk once `GitIssueCommitLink → TaskGitIssueLink` traceability is later wired into scoring; must never be summarized as "the formula is unchanged."
- **CONFIRMED — Standalone evidence unaffected:** `Document` scoring has no Task dependency and was not touched; `findByAuthorIdAndProjectIdAndTaskIsNotNull` already structurally excludes non-task-linked commits (unchanged).
- **CONFIRMED — TEST/RESEARCH have a real evidence source for the first time (Task-marker path only):** a DONE Task with `saga:test`/`saga:research` routes its `taskWeight` into `testScoreByStudent`/`researchScoreByStudent`, feeding real `testContributionScore/Percentage` / `researchContributionScore/Percentage` values — this supersedes DEC-087/088's blanket TBD **only for this evidence path**; provider-sourced TEST/RESEARCH evidence (attachments, traceability-linked commits) remains `TBD_PRODUCT_RULE`.
- **CONFIRMED — Migration `V37__fold_legacy_design_weight_into_document.sql` (new, does not touch V34/V35/V36 — applied status unconfirmed by runtime evidence, safe default is to leave them alone):** product-given exact formula `document = document + design; design = 0` for `course` and `project_group_weight_config`. `code`/`test`/`research` are never written by this migration. **Guard corrected after audit:** the first draft only checked `design ≠ 0`, which is unsafe — neither `CourseContributionWeightService#updateCurrentWeights` nor `ProjectGroupWeightConfigService#update` against an existing row ever zeroes legacy `design`, so a row can already be validly configured with an active 4-field sum of 100/1.0 sitting next to a stale non-zero `design`. Final guard: only fold when `code+test+document+research+design` still equals exactly 100/1.0 (re-tests the pre-DEC-088 validated invariant, not an invented condition) — otherwise the row is left untouched. Proven by an executed test (`LegacyDesignWeightFoldMigrationContractTest`, runs the real SQL against in-memory H2) since string-content assertion alone cannot prove conditional fold/no-fold behavior.
- **Contracts:** OpenAPI **148** (removed 3 legacy Course slice-weight request/list/decision routes). Migration head **V36 → V37**. Peer Review / Rubric / individual override / Student progress authorization / AI Agent / Jira-GitHub sync / ProjectType / COURSE-TEAM mode (DEC-088) **all unchanged**.

## Contribution weight: Course-default + optional exclusive Team override (DEC-088, supersedes DEC-087) — 2026-08-15

- **CONFIRMED_SOURCE_TEST — supersede DEC-087 dưới đây (giữ nguyên văn bản DEC-087 làm lịch sử):** Contribution weight authority nay có đúng hai mode loại trừ nhau trên `Course.contributionConfigMode` (`ContributionConfigMode.COURSE` / `.TEAM`). `COURSE` = mọi Team thuộc Course dùng chung một bộ bốn trọng số `CODE/TEST/DOCUMENT/RESEARCH` (0..100, tổng 100 ± 0.01) của Course. `TEAM` = **mọi** Team hiện tại (chưa xoá) của Course bắt buộc có `ProjectGroupWeightConfig` riêng hợp lệ — không có Team nào được thiếu; thiếu = "TEAM configuration INCOMPLETE", không bao giờ fallback im lặng về Course. **Không có mode thứ ba/hỗn hợp** (Team A dùng override, Team B dùng Course trong cùng Course) — bị cấm tường minh bởi product owner.
- **CONFIRMED — Criteria universe đổi CODE/TEST/DOCUMENT/DESIGN → CODE/TEST/DOCUMENT/RESEARCH:** DESIGN không còn là Contribution criterion active. `ProjectType.DESIGN_ARCHITECTURE` (xem DEC-086) vẫn tồn tại như catalog value nhưng độc lập hoàn toàn — ProjectType không khoá/quyết định Contribution weight. Evidence từng gán DESIGN (`DocumentType.DESIGN` + design-keyword task classification có sẵn) nay remap thẳng vào DOCUMENT (deterministic, không invent taxonomy mới, không mapping DESIGN→RESEARCH nào được tạo ra).
- **CONFIRMED — `ContributionSliceWeightResolver.resolve(Team)` mode-aware, fail-closed:** `COURSE` → luôn `ContributionSliceWeights.fromCourse(team.getCourse())`, mọi Team resolve giống hệt nhau. `TEAM` → tra `ProjectGroupWeightConfig` theo đúng `projectId` và xác nhận đúng Team sở hữu row đó; thiếu hoặc thuộc Team khác → ném `IntegrationException(TEAM_WEIGHT_CONFIG_INCOMPLETE)`, **không bao giờ** fallback về Course. Team mới tạo sau khi TEAM mode đã active cũng phải có override riêng — không tự động thừa hưởng Course weight.
- **CONFIRMED — Mode switch fail-safe, atomic:** `PUT /api/v1/courses/{courseId}/contribution-config-mode` (LECTURER instructor đúng Course, CSRF bắt buộc, không mở ADMIN/Student) — chuyển sang `TEAM` chỉ khi audit xác nhận toàn bộ Team hiện tại đã có override hợp lệ (validate trước, activate sau, không có trạng thái partial); thiếu dù một Team → 409 `TEAM_MODE_CONFIGURATION_INCOMPLETE`, mode giữ `COURSE`. Chuyển về `COURSE` không xoá `ProjectGroupWeightConfig` cũ — giữ historical/inactive, tái dùng được nếu quay lại TEAM mode.
- **CONFIRMED — `PUT /api/projects/{projectId}/group-weights` hồi sinh** (DEC-087 từng xoá hoàn toàn): `ProjectGroupWeightConfigController`/`Service`/request-response DTO quay lại với 4 field `{codeWeight, testWeight, documentWeight, researchWeight}`, storage 0..1 scale (không đổi so với thiết kế gốc, không "sửa" để khớp Course API 0..100 — normalize là trách nhiệm tầng service). Authorization hẹp nguyên bản: ADMIN hoặc đúng LECTURER phụ trách Course sở hữu Team — **không** mở rộng cho Student/Leader dù `ProjectIntegrationAuthorizationService.requireTeamManager` (dùng ở route khác) cho phép Leader; route này cố ý dùng authorization riêng hẹp hơn.
- **CONFIRMED — Course API:** `GET/PUT .../contribution-slice-weights` route không đổi; body/response đổi `designWeight` → `researchWeight`. PUT vẫn LECTURER exact instructor only, CSRF bắt buộc, ADMIN direct PUT vẫn 403. Legacy approve flow: áp code/document trực tiếp, ghi `design` cũ (nếu có) verbatim vào cột inactive `design_contribution_weight`, ép `testContributionWeight=0` — an toàn vì `fromCourse` luôn renormalize theo tổng active slice.
- **CONFIRMED — Team-menu read mới:** `GET /api/v1/courses/{courseId}/contribution-team-weights` (ADMIN/LECTURER) — trước đây không tồn tại dưới bất kỳ tên nào; trả mode hiện tại của Course + effective weight từng Team + nguồn.
- **TEST_SLICE_CLASSIFICATION / RESEARCH_SLICE_CLASSIFICATION = TBD_PRODUCT_RULE:** không có authoritative source/product rule cho testing/QA marker; TEST keyword list ban đầu bị xác định là tự invent và đã gỡ bỏ (bài học đã ghi nhận). RESEARCH cũng chưa có deterministic evidence source ngoài phần đã remap từ DESIGN sang DOCUMENT. `testContributionScore`/`researchContributionScore` trong response luôn `0.0`. **Không tuyên bố 4-tiêu-chí scoring COMPLETE.**
- **CONFIRMED — Migration:** `V35__add_course_contribution_config_mode_and_weights.sql` (đổi tên + viết lại từ bản DEC-087) + `V36__add_test_research_weight_to_project_group_weight_config.sql` (mới) — cả hai chỉ `ADD COLUMN ... DEFAULT 0`, **không UPDATE row nào đã tồn tại**, giữ nguyên giá trị Lecturer đã cấu hình (kế thừa nguyên tắc "no data-resetting migration"). `LEGACY_DESIGN_WEIGHT_MIGRATION = TBD` — không có formula an toàn để tự chuyển design→research; giá trị cũ (`design_contribution_weight`, `design_weight`) giữ lại thuần lịch sử, không đọc bởi resolver mới.
- **Contracts:** OpenAPI operation count **148 (DEC-087) → 151** (hồi sinh 1 PUT `group-weights`, thêm mới 1 PUT `contribution-config-mode` + 1 GET `contribution-team-weights`). Migration head **V35 → V36** (no collision). Peer Review / individual contribution override / Rubric / Student progress authorization / AI Agent / Jira-GitHub / ProjectType selection khi tạo Project **không đổi**. Runtime migration **NOT_YET_UNTIL_DEPLOY**.

## Course-wide 4-slice Contribution weights (DEC-087, SUPERSEDED BY DEC-088 above) — 2026-08-15

- **CONFIRMED_SOURCE_TEST:** Contribution weight authority chuyển thành **Course-only**: mọi Team thuộc một Course dùng chung đúng một bộ bốn trọng số `CODE/TEST/DOCUMENT/DESIGN` (0..100, tổng 100 ± 0.01) của Course đó. `Class` entity không phải authority cho scope này — request `courseId` từ URL vẫn đủ, không thêm `classId`.
- **CONFIRMED — Route giữ nguyên, payload mở rộng:** `GET/PUT /api/v1/courses/{courseId}/contribution-slice-weights` route không đổi; body/response nay có `testWeight` bên cạnh `codeWeight/documentWeight/designWeight`. PUT vẫn LECTURER exact instructor only, CSRF bắt buộc, không `lecturerId`, ADMIN direct PUT vẫn 403 (giữ contract cũ). Legacy request/decision flow không mở rộng testWeight (deprecated); Admin approve ghi testWeight=0 để giữ tổng 100.
- **CONFIRMED — ProjectGroupWeightConfig retired:** `ContributionSliceWeightResolver.resolve(Team)` luôn dùng `Team.course`; không còn đọc `ProjectGroupWeightConfig`. `PUT /api/projects/{projectId}/group-weights` và `ProjectGroupWeightConfigController`/`Service`/DTO bị xóa hoàn toàn — không route thay thế theo Team. Entity + repository giữ nguyên (historical retention, không drop table/data).
- **TEST_SLICE_CLASSIFICATION = TBD_PRODUCT_RULE:** không có `TaskType.TEST`/`DocumentType.TEST` hay bất kỳ authoritative source/product rule nào cho testing/QA marker trước milestone. Một TEST keyword list ban đầu được thêm vào `classifyTaskSlice` bị audit lại và xác định là **tự invent** (không repository/domain nào chứng minh) → đã **gỡ bỏ** khỏi `TeamContributionService` và `ContributionCalculationService`; `classifyTaskSlice` quay lại đúng CODE/DOCUMENT/DESIGN như trước milestone, không có cơ chế nào gán TEST cho Task/commit. **Không tuyên bố TEST evidence scoring COMPLETE.**
- **CONFIRMED — Course `testWeight` vẫn backward-safe:** field `testWeight` trên Course entity/DTO/API vẫn được chấp nhận, lưu, đọc lại bình thường (không phải rollback DTO), nhưng khi tính Contribution nó luôn được coi là slice không có evidence (`normalizeForActiveSlices` nhận `testActive = false` cố định) nên ngân sách được phân bổ lại cho Code/Document/Design theo đúng cơ chế "slice thiếu evidence" đã có sẵn; `testContributionScore`/`testContributionPercentage` trong response luôn `0.0`.
- **CONFIRMED — Migration `V35__add_course_test_contribution_weight.sql` (đã sửa sau audit):** chỉ `ALTER TABLE course ADD COLUMN test_contribution_weight DOUBLE NOT NULL DEFAULT 0`. **Không UPDATE Course row nào đã tồn tại** — `code_contribution_weight`/`document_contribution_weight`/`design_contribution_weight` giữ nguyên giá trị cũ (Lecturer-configured hoặc default cũ), cột mới backfill 0 nên tổng vẫn đúng 100 mà không cần suy diễn công thức redistribute. Course tạo mới sau V35 nhận default `25/25/25/25` từ tầng application (`Course#applyDefaultContributionWeights`, `@PrePersist`), không phải do migration ghi đè.
- **Contracts:** OpenAPI operation count **149 → 148** (contract test PASS, xóa đúng 1 public PUT). Migration head **V34 → V35** (no collision). Peer Review / individual contribution override / Rubric / Student progress authorization / AI Agent / Jira-GitHub / ProjectType selection khi tạo Project **không đổi**. Runtime migration **NOT_YET_UNTIL_DEPLOY**.

## ProjectType fixed canonical catalog (DEC-086) — 2026-08-15

- **CONFIRMED_SOURCE_TEST:** ProjectType không còn dynamic ADMIN-managed catalog; nó là fixed migration-seeded canonical SAGA taxonomy (không phải chuẩn IEEE/ISO). `ProjectTypeController#create` (`POST /api/project-types`) bị xóa; controller chỉ còn `GET`, không có endpoint create/update/delete thay thế.
- **Codes/names cuối cùng (đã audit và chỉnh sửa in-place trong V34):** `DESIGN_ARCHITECTURE`/"Design & Architecture", `RESEARCH`/"Research", `TESTER`/"Tester", `DOCUMENT`/"Document" — 4 UUID literal cố định. ProjectType **không quyết định Contribution weight** (xem DEC-088 — hai khái niệm độc lập).
- **Migration `V34__replace_project_type_with_canonical_catalog.sql`:** null `project.project_type_id` trước, `DELETE FROM project_type`, insert đúng 4 canonical row (codes/names ở trên) với UUID literal cố định trong migration. Không drop table/FK/column, không hard-delete Project. Existing Project mất old ProjectType reference, đọc lại `projectType=null` (legacy-compatible, product-approved reset).
- **CONFIRMED — Project create không đổi:** `POST /api/teams/{teamId}/projects` vẫn bắt buộc `projectTypeId`; thiếu → `PROJECT_TYPE_REQUIRED`; unknown → `PROJECT_TYPE_NOT_FOUND`. FE GET catalog rồi gửi `projectTypeId` đã chọn; không nhận code/name.
- **CONFIRMED — LocalDemoDataSeeder:** resolve canonical `DESIGN_ARCHITECTURE` qua `ProjectTypeRepository.findByCode`; nếu catalog thiếu thì fail rõ (`IllegalStateException`) thay vì tạo ProjectType thứ 5.
- **Contracts:** OpenAPI operation count **150 → 149** (xóa đúng 1 public POST, contract test PASS). Migration head **V33 → V34** (no collision). Contribution formula / Peer Review / Rubric / `ProjectGroupWeightConfig` / AI Agent / Lecturer & Admin Dashboard / Jira/GitHub / Student progress authorization **không đổi**. Runtime DB reset **NOT_YET_UNTIL_DEPLOY**.

## Student progress LEADER exact-Team (DEC-085) — 2026-08-15

- **CONFIRMED_SOURCE_TEST:** `GET .../students/{studentId}/progress` STUDENT LEADER không còn 409 chỉ vì target có thêm TeamMember khác trong cùng Course. Allow khi actor LEADER Team X **và** target có membership Team X. Union nếu actor lead nhiều Team. MEMBER self / teammate 403 giữ. MENTOR 403. Cross-Team 403. Không mở Course-wide. LEADER không global / không Class-wide: request `courseId` + exact Team + `roleInTeam`. `classId` không có trên progress route. ADMIN/LECTURER uniqueness path không đổi. OpenAPI **150**. Migration **V33**. Runtime smoke **TBD**.
...
- **CONFIRMED — Student progress:** existing `GET /api/v1/courses/{courseId}/students/{studentId}/progress`. MEMBER self 200 / teammate 403. LEADER self + exact Team (union các Team actor LEAD) 200; cross-Team/cross-Course 403. MENTOR / no membership 403. ADMIN retained; LECTURER exact instructor. Actor = `SagaPrincipal.localProfileId`. **SUPERSESSION (DEC-085):** target thêm membership ngoài exact Leader Team **không** còn 409. 409 chỉ khi MEMBER self ambiguous hoặc target nằm trên nhiều Team actor cùng lead. Không mở STUDENT cho activities / contribution-detail / early-warnings / Lecturer Dashboard. GET session, no CSRF, no Bearer.
...
- **CONFIRMED — Admin graph-processing DEC-102:** `GET /api/admin/reports/graph-processing` remains ADMIN-only, browser-session GET/no-CSRF/no-Bearer. Successful `CONTRIBUTION`/`INTERACTION` projections persist fail-open UTC runs. The response has `periodDays=7`, `historySupported=true`, nullable `coverageStart`, and only actual `Asia/Ho_Chi_Minh` buckets `{date,nodesBuilt,edgesBuilt,runCount}`; no fake history or zero padding.
