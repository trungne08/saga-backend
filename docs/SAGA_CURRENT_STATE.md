...
- **Đã hoàn thành / CONFIRMED_SOURCE_TEST — supersede toàn bộ section DEC-087 ngay dưới đây** (giữ nguyên văn bản DEC-087 cho lịch sử, không sửa/xoá). Contribution weight authority nay có đúng hai mode loại trừ nhau trên `Course.contributionConfigMode`: **`COURSE`** (mọi Team dùng chung Course weight) hoặc **`TEAM`** (**mọi** Team hiện tại của Course bắt buộc có `ProjectGroupWeightConfig` riêng hợp lệ — thiếu một Team là "chưa hoàn tất", không bao giờ fallback im lặng về Course). Không có mode hỗn hợp "Team override nếu có, không thì Course" — bị cấm tường minh.
- **Criteria universe: CODE/TEST/DOCUMENT/RESEARCH** (không còn DESIGN). DESIGN vẫn là ProjectType catalog value (`DESIGN_ARCHITECTURE`) nhưng độc lập hoàn toàn với Contribution — ProjectType không khoá/quyết định weight. Evidence DESIGN cũ (Document `DESIGN` + task keyword) remap thẳng vào DOCUMENT — deterministic, không invent taxonomy mới.
- **`ContributionSliceWeightResolver` mode-aware, fail-closed:** `COURSE` → luôn `ContributionSliceWeights.fromCourse(team.getCourse())`. `TEAM` → tra đúng `ProjectGroupWeightConfig` theo `projectId` + xác nhận đúng Team sở hữu; thiếu/sai Team → ném `IntegrationException(TEAM_WEIGHT_CONFIG_INCOMPLETE)`, không fallback. Team mới tạo sau khi TEAM mode active cũng phải có override riêng trước khi Contribution tính được cho Team đó.
- **Mode switch fail-safe:** `PUT /api/v1/courses/{courseId}/contribution-config-mode` (LECTURER instructor đúng Course) — chuyển sang `TEAM` chỉ khi audit xác nhận toàn bộ Team hiện tại đã có override hợp lệ (atomic, không có trạng thái partial; thiếu → 409 `TEAM_MODE_CONFIGURATION_INCOMPLETE`). Chuyển về `COURSE` không xoá `ProjectGroupWeightConfig` cũ của Team — giữ lại làm historical/inactive, tái dùng được nếu quay lại TEAM mode.
- **`PUT /api/projects/{projectId}/group-weights` hồi sinh** (DEC-087 từng xoá) — 4 field `{codeWeight, testWeight, documentWeight, researchWeight}`, 0..1 scale (không đổi so với thiết kế gốc), tổng đúng 1.0. Authorization hẹp nguyên bản: ADMIN hoặc đúng LECTURER phụ trách Course — **không** mở cho Student/Leader.
- **Course API:** `GET/PUT .../contribution-slice-weights` đổi `designWeight` → `researchWeight` (0..100 scale, tổng 100). Legacy approve flow ghi `design` cũ verbatim vào cột inactive, ép `testContributionWeight=0`.
- **Team-menu read mới:** `GET /api/v1/courses/{courseId}/contribution-team-weights` (ADMIN/LECTURER) — mode hiện tại + effective weight từng Team + nguồn.
- **Migration:** `V35` (đổi tên, viết lại) + `V36` (mới, cho `project_group_weight_config`) — cả hai chỉ `ADD COLUMN ... DEFAULT 0`, **không UPDATE row cũ nào**, giữ nguyên giá trị Lecturer đã cấu hình. `LEGACY_DESIGN_WEIGHT_MIGRATION = TBD` — không có formula an toàn để tự chuyển design→research, giá trị cũ chỉ còn ý nghĩa lịch sử.
- **Contracts:** OpenAPI **148 (DEC-087) → 151**. Migration head **V35 → V36**. `TEST_SLICE_CLASSIFICATION`/`RESEARCH_SLICE_CLASSIFICATION = TBD_PRODUCT_RULE` — chưa deterministic evidence source; `testContributionScore`/`researchContributionScore` luôn `0.0`; **không tuyên bố 4-tiêu-chí scoring COMPLETE**. Peer Review / Rubric / individual override / Student progress / AI Agent / Jira-GitHub không đổi. Runtime migration **NOT_YET_UNTIL_DEPLOY**.

## Course-wide 4-slice Contribution weights (DEC-087, SUPERSEDED BY DEC-088 above) — 2026-08-15

- **Đã hoàn thành / CONFIRMED_SOURCE_TEST:** Contribution weight authority là **Course-only**. `PUT/GET /api/v1/courses/{courseId}/contribution-slice-weights` giữ route cũ nhưng body/response nay có bốn field `{codeWeight, testWeight, documentWeight, designWeight}` (tổng 100 ± 0.01). Mọi Team thuộc Course dùng chung đúng một bộ trọng số; không còn Project/Team-specific override.
- **ProjectGroupWeightConfig retired khỏi active path:** `ContributionSliceWeightResolver` không đọc `ProjectGroupWeightConfig` nữa — luôn dùng `Team.course`. `PUT /api/projects/{projectId}/group-weights` cùng Controller/Service/DTO bị xóa; entity + repository giữ lại cho historical rows (không drop table). Regression test xác nhận historical row tồn tại vẫn bị bỏ qua.
- **TEST slice classification = TBD_PRODUCT_RULE:** không có authoritative source/product rule cho testing/QA marker. Một keyword list ban đầu bị phát hiện là tự invent và đã **gỡ bỏ**; `classifyTaskSlice` chỉ còn CODE/DOCUMENT/DESIGN như trước. `testWeight` vẫn được Lecturer cấu hình/đọc lại (schema backward-safe) nhưng luôn bị coi là slice không có evidence — ngân sách của nó tự động phân bổ lại cho Code/Document/Design; `testContributionScore/Percentage` trong response luôn `0.0`. **Không tuyên bố TEST scoring hoàn tất.**
- **Migration `V35` (đã sửa):** chỉ `ADD COLUMN test_contribution_weight DOUBLE NOT NULL DEFAULT 0` — **không UPDATE Course row nào đã tồn tại**. Code/Document/Design của Course cũ giữ nguyên; cột mới backfill 0 nên tổng vẫn đúng 100. Course tạo mới sau V35 nhận default `25/25/25/25` từ `Course#applyDefaultContributionWeights` (tầng application), không phải do migration ghi đè.
- **Contracts:** OpenAPI **149 → 148** (xóa đúng 1 public PUT). Migration head **V34 → V35**. Peer Review / individual override / Rubric / Student progress authorization / AI Agent / Jira-GitHub không đổi. Runtime migration **NOT_YET_UNTIL_DEPLOY**.

## ProjectType fixed canonical catalog (DEC-086) — 2026-08-15

- **Đã hoàn thành / CONFIRMED_SOURCE_TEST:** ProjectType không còn dynamic ADMIN catalog. `POST /api/project-types` bị xóa; `ProjectTypeController` chỉ còn `GET`. Catalog seed bởi `V34__replace_project_type_with_canonical_catalog.sql`: null hóa `project.project_type_id`, `DELETE FROM project_type`, insert đúng 4 canonical row (`DESIGN_ARCHITECTURE`/`RESEARCH`/`TESTER`/`DOCUMENT`, codes/names đã audit và chỉnh sửa in-place — xem DEC-088 để phân biệt ProjectType catalog vs Contribution criteria) với UUID literal cố định. Không drop table/FK. Existing Project đọc lại `projectType=null` (product-approved reset, legacy-compatible).
- **Project create không đổi:** `POST /api/teams/{teamId}/projects` vẫn bắt buộc `projectTypeId`; thiếu → `PROJECT_TYPE_REQUIRED`; unknown → `PROJECT_TYPE_NOT_FOUND`. FE phải GET catalog rồi gửi `projectTypeId` đã chọn; không nhận code/name thay UUID.
- **LocalDemoDataSeeder:** resolve canonical `DESIGN_ARCHITECTURE` qua `findByCode`; fail rõ `IllegalStateException` nếu catalog thiếu, không tạo type thứ 5.
- **Contracts:** OpenAPI **150 → 149** (xóa đúng 1 public POST, PASS). Migration head **V33 → V34**. Contribution formula / Peer Review / Rubric / ProjectGroupWeightConfig / AI Agent / Lecturer & Admin Dashboard / Jira/GitHub / Student progress authorization không đổi. Runtime DB reset **NOT_YET_UNTIL_DEPLOY**.
...
# M5 authoritative AI commit-review context (2026-08-14)

- **IMPLEMENTED / CONFIRMED_SOURCE_TEST:** Backend exposes one internal read-only contract: `GET /internal/ai/v1/projects/{projectId}/github/repositories/{repositoryId}/commits/{commitSha}/review-context`.
- **AUTHORITY:** exact Project UUID + GitHub provider repository ID + full commit SHA are verified locally before the existing Backend-owned GitHub client fetches commit files/patch. Only normalized `GitIssueCommitLink -> GitIssue -> TaskGitIssueLink -> Task` relations are returned. Missing links remain `NOT_PROVEN`; no title, Jira-key, issue-number, commit-message, legacy-FK, or AI inference is used.
