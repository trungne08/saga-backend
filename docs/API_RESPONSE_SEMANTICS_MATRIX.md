...
OpenAPI **149 → 150**. Query `sprintId` additive; OpenAPI still **152**. Same 401/403/404 as evaluation. Sprint not on Team Project → 404. No email/cognito on payload.
...
| GET | `/api/v1/teams/{teamId}/contribution-graph` | TeamContributionController | OAS: 200, 401, 403, 404 | TBD | TBD | TBD | OAS | OAS | OAS | OAS | TBD | TBD | getContributionGraph; cùng auth evaluation; query sprintId | OK | Flowchart SAGA; sprintId lọc Sprint. |
